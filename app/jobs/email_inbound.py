...
def _attachments(msg) -> list[tuple[str, bytes, str]]:
    ...
    for part in msg.walk():
        ...
        if not payload:
            continue
        name = _decode(part.get_filename()) or f"upload_{uuid.uuid4().hex}"
        out.append((name, payload, mime))
    return out


def _fetch_matching(lead_emails: set[str]) -> list[dict]:
    """Return parsed messages from known lead addresses (UNSEEN), WITHOUT marking
    them read. Non-matching mail is only header-peeked, never touched."""
    matches: list[dict] = []
    ...
    try:
        ...
        if typ != "OK" or not data or not data[0]:
            return matches
        uids = data[0].split()[-_MAX_PER_POLL:]
        for uid in uids:
            ...
            if sender not in lead_emails:
                continue  # not a client — leave it untouched (unread)
            ...
            matches.append({
                ...
                "message_id": head.get("Message-ID") or f"<{uuid.uuid4().hex}@inbound>",
                "attachments": atts,
            })
        return matches
    finally:
...
async def poll_inbound() -> dict:
    ...
    matches = await asyncio.to_thread(_fetch_matching, set(lead_emails))
